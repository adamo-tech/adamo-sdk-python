"""Adamo video streaming — thin Python wrapper around the native adamo_video module.

Adds ROS 2 topic support on top of the Rust Robot/VideoTrack classes.
ROS sources use rclpy (lazy import, not a hard dependency) to subscribe
to sensor_msgs/Image topics and push frames through the existing push path.
"""
...
class Robot:
    """Adamo robot client — streams video and data via MoQ.
    ...
    def send(self, topic: str, data) -> None:
        self._rust.send(topic, data)
